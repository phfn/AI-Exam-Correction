	...
	return (
		<div className="Site">
			...
			<div className="App">
				{state === 0 && (
					<TaskSelector
						...
						setStudentExams={(examImages, filenames) => {
							const newExamContainer = examContainer.clone()
							newExamContainer.studentExams = examImages.map(
								(image, index) => {
									return new Exam(
										image,
										filenames[index],
										examContainer.correctExam.tasks
									)
								}
							)
							setExamContainer(newExamContainer)
				...
				{state === 2 && (
					<ReviewExam
						exam={examContainer.studentExams[selectedExamIndex]}
						setExam={(newExam) => {
							setExam(selectedExamIndex, newExam)
						}}
						goBack={() => {
							setState(state - 1)
						}}
